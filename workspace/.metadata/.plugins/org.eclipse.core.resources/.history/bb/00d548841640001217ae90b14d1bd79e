...
public class Main {
	...
	private static void geneticAlgo(Organism[] population){
		...
		while(!hasGoal(population)){ //testing on whether the population has any goal in it
			...
			keepers += 0.05; //for each iteration of a new population, increase the rank at which we will accept a fitness
		}
	...
	private static boolean hasGoal(Organism[] population){
		Organism fittest = new Organism(numPegs); //create new organism to hold fittest individual
		fittest = population[0]; //preliminary set of fittest inidividual
		for(int i = 0; i < population.length; i++){
			if(population[i].getFitness() >= 1) //if our fitness is 1, then we have a goal
				return true;
			if(fittest.getFitness() < population[i].getFitness()) //if the fittest so far is less then the current population 
				fittest = population[i]; //keeping track of the current fittest creature
		}
		if(fittest.getFitness() >= 0.90) //if we have an organism that has a fitness greater than 90% assume solution
			return true;
		return false;
	}//end method
